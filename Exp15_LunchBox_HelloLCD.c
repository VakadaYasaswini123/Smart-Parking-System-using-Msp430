...
void main(void)
{
    ...
    P2DIR &= ~SW1;                   // Set SW pin -> Input
    P2DIR &= ~SW2;                   // Set SW pin -> Input
    P2DIR &= ~SW3;                   // Set SW pin -> Input
    ...
    lcd_setCursor(2,0);             // Cursor position (1,3)
        lcd_print("                                ");          // Print
        lcd_setCursor(0,1);             // Cursor position (0,1)
            lcd_print(" S1      S2   ");    // Print
    while(1)
    {
        if(P2IN & SW1)               // If SW is Pressed
        {
            lcd_setCursor(1,0);             // Cursor position (0,1)
            lcd_print(" NTV");    // Print
    }
        else{
            lcd_setCursor(1,0);             // Cursor position (0,1)
        ...
        if(P2IN & SW2)               // If SW is Pressed
                {
                    lcd_setCursor(1,5);             // Cursor position (0,1)
                    lcd_print("    NTV ");    // Print
            }
        else{
                    lcd_setCursor(1,5);             // Cursor position (0,1)
